...
APP_NAME=${APP_NAME:-"iot-service-api"}
NAMESPACE=${NAMESPACE:-"iot"}
ENV_SECRET_NAME=${ENV_SECRET_NAME:-"iot-api-secrets"}
DEPLOY=${DEPLOY:-true}
SETUP_DATABASES=${SETUP_DATABASES:-true}
DB_TYPES=${DB_TYPES:-postgres,redis}
# Per-service database configuration
SERVICE_DB_NAME=${SERVICE_DB_NAME:-iot}
SERVICE_DB_USER=${SERVICE_DB_USER:-iot_user}
...
# For local builds: docker build -f iot-service/Dockerfile -t iot-service:local .
...
if [[ -d "../shared/auth-client" ]]; then
  ...
  DOCKER_BUILDKIT=1 docker build -f iot-service/Dockerfile -t "${IMAGE_REPO}:${GIT_COMMIT_ID}" .
fi
