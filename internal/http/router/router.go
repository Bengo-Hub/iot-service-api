...
import (
	...
	sharedmw "github.com/bengobox/iot-service/internal/shared/middleware"
)
...
func New(log *zap.Logger, health *handlers.HealthHandler, userHandler *handlers.UserHandler, authMiddleware *authclient.AuthMiddleware, allowedOrigins []string) http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Route("/{tenantID}", func(tenant chi.Router) {
			...
			tenant.Route("/devices", func(devices chi.Router) {
				// Placeholder endpoints - to be implemented
				devices.Get("/", func(w http.ResponseWriter, r *http.Request) {
					w.WriteHeader(http.StatusNotImplemented)
					w.Write([]byte("Not implemented yet"))
				})
			})

			tenant.Route("/telemetry", func(telemetry chi.Router) {
				// Placeholder endpoints - to be implemented
				telemetry.Get("/", func(w http.ResponseWriter, r *http.Request) {
					w.WriteHeader(http.StatusNotImplemented)
					w.Write([]byte("Not implemented yet"))
				})
			})
